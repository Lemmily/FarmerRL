...
class Object:
    ...
    def clear(self, camx=0, camy=0):
        # erase the character that represents this object
        libtcod.console_put_char(R.con_char, self.x - camx, self.y - camy, " ", libtcod.BKGND_NONE)

    def draw(self, camx=0, camy=0):

        if camx <= self.x < camx + R.MAP_VIEW_WIDTH and camy <= self.y < camy + R.MAP_VIEW_HEIGHT:
            posx = self.x - camx
            posy = self.y - camy

            #                 libtcod.console_put_char(R.con, posx, posy, " ")
            libtcod.console_set_default_foreground(R.con_char, self.colour)
            #                 libtcod.console_put_char_ex(R.con_char, posx, posy, self.char, self.colour, self.colour)
            libtcod.console_put_char(R.con_char, posx, posy, self.char, libtcod.BKGND_NONE)
...
plant_types = {
    # numbering is wrong
    # name : 0_produce type, 1_characters, 2_daystoharvest, 3_daystofullharvest, 4_daysinfullharvest, 5_MaxCrop, 6_DC]
    "corn": ["vegetable", [":", "!"], "spring", 50, 25, 15, 2, 5],
    "tomato": ["vegetable", ["s", "$"], "spring", 50, 10, 20, 10, 2],
    "peas": ["vegetable", ["s", "$"], "spring", 50, 25, 15, 8, 5],
    "apple": ["fruit", ["t", "T"], "autumn", 1095, 25, 15, 30, 15],
    "pear": ["fruit", ["t", "T"], "autumn", 1460, 25, 15, 30, 20 ],
}


class Seed(Object):
    def __init__(self, type_):
        Object.__init__(self, type_)
        self.type = type_
...
class Plant(Object):
    def __init__(self, type_, x, y):
        Object.__init__(self, x, y, type_)
        self.age = 0
...
# this could also be kept in a lookup table, with the plant object just holding the type and
...
class Stats:
    ...
    def gain_exp(self, amount, skill):
        # TODO: exp gets split into a chunk for skill used, and the rest gets divided between the rest based on priority
...
class Skill:
    def __init__(self, name, group="None"):
        ...
        self.exp = 0
        self.level = 1
        # TODO: racial or class based aptitude - could even be affected by high skill level.
        self.aptitude = 1  # speed at which they gain skill levels.... probably somewhere better to hold this.

    def gain(self, amount):
        self.exp += amount * self.aptitude
        if self.exp > 100:
            self.level += 1
            self.exp -= 100
...
def act_till(entity):
    ...
    if entity.stats and entity.stats.has("tilling"):
        ...
        if skill >= tile.difficulty:
            ...
            if tile.till(amount):
                entity.stats.gain_exp(tile.difficulty * (tile.difficulty / entity.stats.skills["tilling"].level),
                                      "tilling")
        else:
            print "you failed a", dc, " with a skill of ", skill
